...
class NewAgent(Agent):
    ...
    def __init__(self):
        ...
        # ============ 创新点：时序规划参数 (Beam Search) ============
        self.SEARCH_DEPTH = 2       # 搜索深度 (2表示看当前杆+下一杆)
        self.BEAM_WIDTH = 3         # 搜索广度 (每层保留最好的3个分支)
        self.GAMMA = 0.6            # 折扣因子 (更看重当前得分)
        self.LOOKAHEAD_ENABLED = True
        
        print("[NewAgent] 融合版 v5 (带时序规划) 初始化完成")
    ...
    # ============================================================================
    # 时序规划 (Beam Search / Lookahead) 核心逻辑
    # ============================================================================

    def _simulate_and_get_state(self, action, balls, table):
        """模拟击球并返回结束后的状态（用于推演未来）"""
        sim_balls = {bid: copy.deepcopy(ball) for bid, ball in balls.items()}
        sim_table = copy.deepcopy(table)
        shot = pt.System(table=sim_table, balls=sim_balls,
                        cue=pt.Cue(cue_ball_id="cue"))
        ...
        return shot, sim_balls

    def _recursive_search_score(self, balls, my_targets, table, depth):
        """
        递归评估函数 (DFS with Beam Pruning)
        返回: 该局面下的最佳未来期望得分 (Future Value)
        """
        # 1. 终止条件
        if depth == 0:
            return 0
        ...
        if not remaining:
            if '8' in balls and balls['8'].state.s != 4:
                return 0 # 还需要打黑8，不算完全结束，交给下一层评估
            # 已经赢了 (注意：这里简化处理，假设最后打进8就赢)
            return 500
        ...
        # 2. 生成候选 (Beam Filter)
        # 复用 _choose_top_targets 进行快速几何筛选
        # 注意：这里我们只关心几何可行性，不关心特别细致的防守逻辑
        cue_pos = balls['cue'].state.rvw[0]
        
        next_candidates = self._choose_top_targets(
            balls, remaining if remaining else ['8'], table, 
            num_choices=self.BEAM_WIDTH
        )
        ...
        # 3. 遍历分支
        for choice in next_candidates:
            # 生成动作
            target_pos = balls[choice['target_id']].state.rvw[0]
            pocket_pos = table.pockets[choice['pocket_id']].center
            
            if choice['type'] == 'bank':
                action = self._geo_bank_shot(cue_pos, target_pos, pocket_pos, choice['cushion_id'])
            elif choice['type'] == 'curve':
                action = self._geo_shot(cue_pos, target_pos, pocket_pos)
                action['a'] = self._calc_curve_a(choice['cut_angle'])
            else:
                action = self._geo_shot(cue_pos, target_pos, pocket_pos)
            
            # 模拟一步
            shot_res, next_state_balls = self._simulate_and_get_state(action, balls, table)
            
            if shot_res is None:
                continue
                
            # 计算这一步的即时得分 (Immediate Reward)
            immediate_score = self._improved_reward_function(shot_res, balls, my_targets, table)
            
            # 剪枝：如果这一步已经是臭棋（犯规或得分太低），就不要往下推了
            if immediate_score < -20: 
                continue
            
            # 递归调用下一层
            future_val = self._recursive_search_score(
                next_state_balls, my_targets, table, depth - 1
            )
            
            # Bellman Equation: V = R + gamma * V_next
            total_val = immediate_score + self.GAMMA * future_val
            
            if total_val > best_future_val:
                best_future_val = total_val
                
        return best_future_val if best_future_val != -float('inf') else -50
        ...
        try:
            ...
            for choice in top_choices:
                ...
                if self.LOOKAHEAD_ENABLED:
                    # 1. 模拟当前这一步
                    shot_res, next_balls = self._simulate_and_get_state(base_action, balls, table)
                    ...
                    if shot_res:
                        ...
                        # 3. 如果即时得分还行，推演未来
                        if immediate_score > -50:
                            future_score = self._recursive_search_score(
                                next_balls, my_targets, table, self.SEARCH_DEPTH - 1
                            )
                            # 总分 = 即时 + 折扣 * 未来
                            combined_score = immediate_score + self.GAMMA * future_score
                            geo_evaluated.append((base_action, combined_score, choice))
                        else:
                            # 即使这杆打得烂，也要加入列表，只不过分低
                            geo_evaluated.append((base_action, immediate_score, choice))
                    else:
                         geo_evaluated.append((base_action, -100, choice))
                else:
                    # 原有逻辑：只看当前
